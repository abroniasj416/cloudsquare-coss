...
![Cloudsquare COSS LMS Video 아키텍처](./image/아키텍처.png)
...
## 8. Completion Certificate 데모

### Serial Number 규칙
- 형식: "lectureId-studentId"
- 각 ID는 4자리 0-padding
- 예시: `lectureId=1`, `studentId=2` -> "0001-0002"

### API
- `POST /api/lectures/{lectureId}/complete`
  - 현재 로그인 사용자 기준 수강 완료 처리 + 수료증 발급
  - 이미 발급된 경우 기존 수료증 반환
- `GET /api/certificates/me`
  - 현재 로그인 사용자 수료증 목록 조회
- `GET /api/certificates?userId={id}`
  - External 연동용 데모 API
  - 운영 환경에서는 내부망/방화벽/ACG 제한 전제
- `GET /api/certificates/lectures/{lectureId}/me`
  - 특정 강의의 내 수료증 조회

### Demo 시나리오
1. 학생 계정으로 로그인
2. `Lectures` -> 강의 상세 -> `Complete Lecture` 클릭
3. 발급된 `serialNumber` 확인
4. 상단 `Certificates` 메뉴에서 내 수료증 목록 확인